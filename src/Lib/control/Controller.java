...
//		System.out.println("========7=======");
	...
	public JSONObject lookupUserHistory(JSONObject object) {
		...
		for(int i =0;i<hislist.size();i++) {
			...
			if(hislist_continue.get(hislist_continue.size()-1).getReturnDay()!=null&&hislist.get(i).getReturnDay()!=null&&hislist.get(i).getBid().equals(hislist_continue.get(hislist_continue.size()-1).getBid())&&hislist.get(i).getBorrowDay().toString().equals(hislist_continue.get(hislist_continue.size()-1).getReturnDay().toString())
					) {
